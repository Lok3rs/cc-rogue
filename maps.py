from components import Entity, Item, Monster, Player, GameMap, Chamber
...
MAP_A = GameMap(MAP_A_CHAMBERS, MAP_A_ENTITIES).generate_map()
...
MAP_B_PLAYER = Player(START_MAP_B['start_x'], START_MAP_B['start_y'])
...
MAP_B_ENTITIES = {
    ...
    Item("special", "golden key", bonus=1).put_on_map(MAP_B_CHAMBERS["room_1"]),
}

MAP_B = GameMap(MAP_B_CHAMBERS, MAP_B_ENTITIES).generate_map()
...
MAP_C_PLAYER = Player(START_MAP_C['start_x'], START_MAP_C['start_y'])
...
MAP_C_ENTITIES = {
    ...
    Item("special", "rusty key").put_on_map(MAP_C_CHAMBERS["room_1"])
}

MAP_C = GameMap(MAP_C_CHAMBERS, MAP_C_ENTITIES).generate_map()
